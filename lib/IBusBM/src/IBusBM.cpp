...
#if defined(ARDUINO_ARCH_ESP32)
static hw_timer_t *IBusBMtimer = NULL;

static uint8_t IBusBMclampRxFifoFullThreshold(uint8_t threshold) {
  if (threshold == 0) return 1;
  if (threshold > 127) return 127;
  return threshold;
}
#endif
...
void IBusBM::begin(HardwareSerial &serial, int8_t timerid, int8_t rxPin, int8_t txPin) {
  begin(serial, timerid, rxPin, txPin, IBUSBM_ESP32_RX_BUFFER_SIZE, IBUSBM_ESP32_TX_BUFFER_SIZE, IBUSBM_ESP32_RX_FIFO_FULL_THRESHOLD);
}

void IBusBM::begin(HardwareSerial &serial, int8_t timerid, int8_t rxPin, int8_t txPin, size_t rxBufferSize, size_t txBufferSize, uint8_t rxFifoFullThreshold) {
  #ifdef ARDUINO_ARCH_ESP32
    #if ESP_ARDUINO_VERSION_MAJOR >= 3
      if (rxBufferSize > 0) serial.setRxBufferSize(rxBufferSize);
      if (txBufferSize > 0) serial.setTxBufferSize(txBufferSize);
      if (rxPin >= 0 || txPin >= 0) serial.setPins(rxPin, txPin);
      serial.begin(115200, SERIAL_8N1, -1, -1, false, 20000UL, IBusBMclampRxFifoFullThreshold(rxFifoFullThreshold));
    #else
      (void)rxBufferSize;
      (void)txBufferSize;
  ...
  if (!IBusBMfirst && timerid != IBUSBM_NOTIMER) {
      ...
      #if defined(ARDUINO_ARCH_ESP32) 
        #if ESP_ARDUINO_VERSION_MAJOR >= 3
          (void)timerid; // Core 3.x allocates the timer; the numeric id is kept for API compatibility.
          IBusBMtimer = timerBegin(1000000);
          if (IBusBMtimer) {
            timerAttachInterrupt(IBusBMtimer, &onTimer);
            timerAlarm(IBusBMtimer, 1000, true, 0);
          }
        #else
          IBusBMtimer = timerBegin(timerid, F_CPU / 1000000L, true); // defaults to timer_id = 0; divider=80 (1 ms); countUp = true;
          timerAttachInterrupt(IBusBMtimer, &onTimer, true); // edge = true
          timerAlarmWrite(IBusBMtimer, 1000, true);  //1 ms
          timerAlarmEnable(IBusBMtimer);
        #endif
      #elif defined(_VARIANT_ARDUINO_STM32_)
